...
import EditApartmentModal from '../../components/Modal/EditApartmentModal';
import ViewApartmentModal from '../../components/Modal/ViewApartmentModal';
...
const ApartmentManagement = () => {
  ...
  const filteredApartments = apartments.filter(apartment => {
    const matchesSearch = searchTerm === '' ||
      String(apartment.number).includes(searchTerm);

    const matchesStatus = statusFilter === '' || apartment.status === statusFilter;
    const matchesType = apartmentTypeFilter === '' || apartment.apartment_type === apartmentTypeFilter;

    // Проверка на соответствие блоку
    const matchesBuilding = buildingFilter === '' || String(apartment.building_id) === buildingFilter;
    ...
    return matchesSearch && matchesStatus && matchesBuilding && matchesType && matchesComplex;
  });
        ...
        <div className={styles.contentArea}>
          <div className={styles.controlsBar}>
            ...
            <select
              ...
              onChange={(e) => setApartmentTypeFilter(e.target.value)}
              className={styles.filterSelect}
            >
          ...
          {loading ? (
            ...
            <div className={styles.loading}>Квартиры не найдены</div>
          ) : (
                  ...
                  {filteredApartments.map((apartment) => (
                    <tr key={apartment.id}>
                      ...
                      <td
                        ...
                        style={{ cursor: 'pointer' }}
                      >
                        {getBuildingName(apartment.building_id)}
                      </td>
                      <td
                        onClick={() => handleView(apartment.id)}
                        style={{ cursor: 'pointer' }}
                      ...
                      <td
                        ...
                        style={{ cursor: 'pointer' }}
                      >
                        {getApartmentTypeLabel(apartment.apartment_type)}
                      </td>
                      <td
                        onClick={() => handleView(apartment.id)}
                        style={{ cursor: 'pointer' }}
                      >
                        {apartment.apartment_area} м²
                      </td>
                      <td
                        onClick={() => handleView(apartment.id)}
                        style={{ cursor: 'pointer' }}
                      >
                        {apartment.price_per_sqr.toLocaleString()} ₸
                      </td>
                      <td
                        onClick={() => handleView(apartment.id)}
                        style={{ cursor: 'pointer' }}
                      >
                        {apartment.total_price.toLocaleString()} ₸
                      </td>
                      <td
                        onClick={() => handleView(apartment.id)}
                        style={{ cursor: 'pointer' }}
                      >
                        <span style={{
                          backgroundColor: getStatusColor(apartment.status),
                          color: '#fff',
                          padding: '6px 12px',
                          borderRadius: '12px',
                          fontSize: '14px',
                      ...
                      <td onClick={(e) => e.stopPropagation()}>
                        <div className={styles.actions}>
                          <button className={styles.actionBtn} onClick={() => handleEdit(apartment.id)}>
                            <svg width="20" height="20" viewBox="0 0 20 20" fill="none">
                              <path d="M14 2L18 6L7 17H3V13L14 2Z" stroke="currentColor" strokeWidth="2"/>
                            </svg>
                          </button>
                          <button className={styles.actionBtn} onClick={() => handleDelete(apartment.id)}>
                            <svg width="20" height="20" viewBox="0 0 20 20" fill="none">
