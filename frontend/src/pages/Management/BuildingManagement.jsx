...
import CreateBuildingModal from '../../components/Modal/CreateBuildingModal';
import EditBuildingModal from '../../components/Modal/EditBuildingModal';
import ViewBuildingModal from '../../components/Modal/ViewBuildingModal';
...
const BuildingManagement = () => {
  ...
  const getComplexName = (complexId) => {
    ...
    return complex?.name || `ЖК #${complexId}`;
  };

  // Фильтрация блоков
  const filteredBuildings = buildings.filter(building => {
    const matchesSearch = searchTerm === '' || 
      String(building.block).includes(searchTerm) ||
    ...
    const matchesComplex = complexFilter === '' || String(building.residential_complex_id) === complexFilter;

    return matchesSearch && matchesStatus && matchesComplex;
  });

  // Группировка блоков по ЖК
  const groupedBuildings = filteredBuildings.reduce((acc, building) => {
    const complexId = building.residential_complex_id;
    if (!acc[complexId]) {
      acc[complexId] = [];
    }
    acc[complexId].push(building);
    return acc;
  }, {});
        ...
        <div className={styles.contentArea}>
          <div className={styles.controlsBar}>
            <div className={styles.searchBox}>
              ...
              <input
                type="text"
                placeholder="Поиск по номеру блока или описанию..."
                value={searchTerm}
          ...
          {loading ? (
            ...
            <div className={styles.loading}>Блоки не найдены</div>
          ) : (
                  ...
                  {filteredBuildings.map((building) => (
                    <tr key={building.id}>
                      ...
                      <td
                        ...
                        style={{ cursor: 'pointer' }}
                      >
                        {building.gross_area.toLocaleString()} м²
                      </td>
                      <td
                        onClick={() => handleView(building.id)}
                        style={{ cursor: 'pointer' }}
                      ...
                      <td onClick={(e) => e.stopPropagation()}>
                        <div className={styles.actions}>
                          <button className={styles.actionBtn} onClick={() => handleEdit(building.id)}>
                            <svg width="20" height="20" viewBox="0 0 20 20" fill="none">
                              <path d="M14 2L18 6L7 17H3V13L14 2Z" stroke="currentColor" strokeWidth="2"/>
                            </svg>
                          </button>
                          <button className={styles.actionBtn} onClick={() => handleDelete(building.id)}>
                            <svg width="20" height="20" viewBox="0 0 20 20" fill="none">
