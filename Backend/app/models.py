...
class Building(Base):
    ...
    def images (self):
        ...
        return session.query (Image).filter (
            Image.object_id == self.id,
            Image.object_type == ObjectType.building
        ).all ()
...
class Apartment(Base):
    ...
    def images (self):
        ...
        return session.query (Image).filter (
            Image.object_id == self.id,
            Image.object_type == ObjectType.apartment
        ).all ()
...
class CommercialUnit(Base):
    ...
    def images (self):
        ...
        return session.query (Image).filter (
            Image.object_id == self.id,
            Image.object_type == ObjectType.commercial
        ).all ()
...
class Image(Base):
    ...
    def related_object (self):
        ...
        if self.object_type == ObjectType.apartment:
            return session.query (Apartment).filter (Apartment.id == self.object_id).first ()
        elif self.object_type == ObjectType.commercial:
            return session.query (CommercialUnit).filter (CommercialUnit.id == self.object_id).first ()
            ...
            return session.query (ResidentialComplex).filter (ResidentialComplex.id == self.object_id).first ()
        return None
