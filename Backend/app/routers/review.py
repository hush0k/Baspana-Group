...
from app.cruds.review import create_review, delete_review, get_average_rating, get_review_by_id, get_review_filtered, \
	update_review
from app.database import get_db
from app.models import Review, Role, User
from app.schemas import PaginatedReviewResponse, ReviewCreate, ReviewResponse, ReviewUpdate
...
def get_review_endpoint(
	...
	if review is None:
		raise HTTPException(status_code = http_status.HTTP_404_NOT_FOUND, detail = "Not existing review")
	return review

@router.get("/complex/{review_id}/average-rating")
def get_complex_average_rating_endpoint(
		complex_id: int,
		db: Session = Depends(get_db)
):
...
def update_review_endpoint(
	...
	if review.user_id != current_user.id and current_user.role not in [Role.admin, Role.manager]:
...
def delete_review_endpoint(
	...
	if existing_review.user_id != current_user.id and current_user.role not in [Role.admin, Role.manager]:
		raise HTTPException(status_code = http_status.HTTP_403_FORBIDDEN, detail = "Not authorized to delete review")
