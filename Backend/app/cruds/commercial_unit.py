...
def get_commercial_by_number(db: Session, number: int, building_id: int):
    return (
        db.query(CommercialUnit)
        .filter(CommercialUnit.number == number)
        .filter(CommercialUnit.building_id == building_id)
        .first()
    )
...
def create_commercial(db: Session, commercialUnit: CommercialUnitCreate):
    if commercialUnit.total_price is None:
        ...
        if (
            commercialUnit.total_price <= min_price
            or commercialUnit.total_price >= max_price
        ):
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"CommercialUnit price must in range {min_price} to {max_price}",
            )
...
def update_commercial(
    db: Session, commercial_id: int, commercial_update: CommercialUnitUpdate
):
